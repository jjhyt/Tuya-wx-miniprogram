...
//发送指令
  ...
  goStatiPage: function(){
    const {device_id} = this.data
    wx.navigateTo({
      url: `/pages/home_center/statistics_panel/index?device_id=${device_id}`,
    })
  },

  CountdowmShow: function(){
    this.setData({
      timeshow:true,
    })
  },
  //发送倒计时命令
  sendCountdowm: function (value) {
    const { device_id } = this.data
    deviceControl(device_id, "countdown_1", value)
  },
  ontimeshowConfirm: function(event){
    this.setData({
      timeshow:false,
    })
    var timeString = event.detail
    var fengeString = timeString.split(":")
    var shiInt = parseInt(fengeString[0])
    var fengInt = parseInt(fengeString[1])
    var value = shiInt * 3600 + fengInt * 60
    console.log(value)
    this.sendCountdowm(value)
  },
